...
class Environment:
    ...
    def init_map(self):
        ...
        for i in range(self.offset, self.env_width - self.offset):
            ...
            for j in range(self.offset, self.env_height - self.offset):
                ...
                if (self.map_img_agents[i, j, 0] < self.food_spawn_threshold) and (rand <= 1):  # TODO (rand == 0)
                    # Set pixel red
    ...
    def move_food(self):
        ...
        for i in range(self.offset, self.env_width - self.offset):
            ...
            for j in range(self.offset, self.env_height - self.offset):
                ...
                if (self.map_img_agents[i, j, 1]) == 255 and (random.randrange(0, 100) < 5):
                    # Create a new random food position
                    x = i + random.randrange(-1, 2)
                    y = j + random.randrange(-1, 2)

                    lower_inside_bounds_x = (x > self.offset)
                    lower_inside_bounds_y = (y > self.offset)
                    upper_inside_bounds_x = (x < self.env_width - self.offset)
                    upper_inside_bounds_y = (y < self.env_height - self.offset)
                    is_within_bounds = lower_inside_bounds_x and lower_inside_bounds_y and upper_inside_bounds_x and upper_inside_bounds_y

                    if is_within_bounds:
                        # Un-paint the agent
                        self.map_img_agents[i, j, 0] = self.map_img_calculations[i, j, 0]
                        self.map_img_agents[i, j, 1] = self.map_img_calculations[i, j, 1]
                        self.map_img_agents[i, j, 2] = self.map_img_calculations[i, j, 2]
                        # Paint the agent to it's new position
                        self.map_img_agents[x, y, 0] = 0
                        self.map_img_agents[x, y, 1] = 255
                        self.map_img_agents[x, y, 2] = 0
    ...
    def step(self, action):
        ...
        if action == 0:  # UP
            ...
            self.calculate_reward(2, 0)
            self.movement_decision(2, 0)
        elif action == 9:
            self.calculate_reward(2, 1)
            self.movement_decision(2, 1)
        elif action == 10:
            self.calculate_reward(2, 2)
            self.movement_decision(2, 2)
        elif action == 11:
            self.calculate_reward(1, 2)
            self.movement_decision(1, 2)
        elif action == 12:
            self.calculate_reward(0, 2)
            self.movement_decision(0, 2)
        elif action == 13:
            self.calculate_reward(-1, 2)
            self.movement_decision(-1, 2)
        elif action == 14:
